...
<div class="card shadow">
    <div class="card-header border-0">
        <div class="row align-items-center">
            <div class="col">
                <h3 class="mb-0">
                    <i class="fas fa-edit text-primary"></i> Editar Carrera
                </h3>
            </div>
    ...
    <div class="card-body">
        <form action="{{ route('carreras.update', $carrera->id) }}" method="POST" id="editForm">
            @csrf
            @method('PUT')
            ...
            <div class="form-row">
                <div class="form-group col-md-6">
                    ...
                    <select name="facultad_id" id="facultad_id" 
                        ...
                        @foreach($facultades as $facultad)
                            <option value="{{ $facultad->id }}" {{ old('facultad_id', $carrera->facultad_id) == $facultad->id ? 'selected' : '' }}>
                ...
                <div class="form-group col-md-6">
                    ...
                    <select name="status" id="status" 
                        ...
                        <option value="active" {{ old('status', $carrera->status) == 'active' ? 'selected' : '' }}>Activo</option>
                        <option value="inactive" {{ old('status', $carrera->status) == 'inactive' ? 'selected' : '' }}>Inactivo</option>
                    </select>
                    @error('status')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
...
<script>
    $(document).ready(function() {
        ...
        $('#editForm').on('submit', function(e) {
            ...
            setTimeout(function() {
                $btn.html('<i class="fas fa-save"></i> Actualizar Carrera').prop('disabled', false);
            }, 3000);
        });
